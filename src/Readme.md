...
The server-side component of Velox consists of a PHP library built as a Composer project. This project can be imported
as a Composer dependency by including the following in the composer.json file in your project and then running the
...
The class structure is divided into a set of sub-namespaces underneath the `KitsuneTech\Velox` base, as reflected in the 
structure in this directory. (There is one exception: the Support directory, which contains independent helper functions
and constants that are not part of the class structure.) Each of these sub-namespaces handles a different facet of the
...
The `Transport` sub-namespace contains one primary class: `Export`. This is used in combination with one or more
